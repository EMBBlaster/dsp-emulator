...
type
  tdeco16_sprite=class
    constructor create(gfx,pant:byte;global_x_size,color_add:word;invert:boolean=false);
    destructor free;
    public
...
constructor tdeco16_sprite.create(gfx,pant:byte;global_x_size,color_add:word;invert:boolean=false);
begin
...
for f:=0 to $ff do begin
      ...
      y:=(240-y) and $1ff;
			nchar:=self.ram[(f*4)+1] and not(multi);
      if fy then inc:=-1
        else begin
						nchar:=nchar+multi;
						inc:=1;
...
procedure deco16_snd_double_init(cpu_clock,sound_clock:dword;sound_bank:cpu_outport_call);
begin
h6280_0:=cpu_h6280.create(cpu_clock);
