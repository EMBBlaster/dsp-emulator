...
case direccion of
  0..7:begin
            ...
            case direccion of
              3:if (valor and 8)<>0 then copymemory(@K007121_chip[video_circuit].sprite_ram[0],@page_ram[video_circuit,$1000],$800)
  ...
  $410:begin
        ...
        video_circuit:=(valor and $40) shr 6;
	      if (valor and $10)<>0 then bank_rom:=(valor and $e) shr 1
...
for chip:=0 to 1 do begin
		for pal:=0 to 7 do begin
			...
			for i:=0 to $ff do begin
				if (((pal and 1)=0) and (memoria_temp[(clut shl 8) or i]=0)) then ctabentry:=0
				  else ctabentry:=(pal shl 4) or (memoria_temp[(clut shl 8) or i] and $f);
